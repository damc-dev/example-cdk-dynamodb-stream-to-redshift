...
BEGIN
    ...
    sql := 'SELECT MAX(approximateArrivalTimestamp) FROM member;';
	...
	END IF;
   	RAISE INFO 'Last approximate_arrival_timestamp inserted into member: %', max_approximate_arrival_timestamp;
    ...
    EXECUTE 'INSERT INTO member_stage ('||
    	' SELECT LTRIM(pk, ''M_'' ) as memberId, sk as memberName, approximateArrivalTimestamp, sequenceNumber, eventName'||
    	' FROM member_quest_data_extract'||
        ' WHERE pk LIKE ''M\\_%'' AND approximateArrivalTimestamp > '''||max_approximate_arrival_timestamp||''');';
    ...
    EXECUTE 'DELETE FROM member_stage where (memberId, approximateArrivalTimestamp) NOT IN (SELECT memberId, MAX(approximateArrivalTimestamp) as approximateArrivalTimestamp FROM member_stage GROUP BY memberId)';
...
SELECT count(*), MIN(approximateArrivalTimestamp), MAX(approximateArrivalTimestamp) FROM member;
